...
(cd "target/${TARGET}/release" && \
    zip -q "/dist/cli/graphdblite-${VERSION}-${TARGET}.zip" graphdblite.exe)
sha256sum "/dist/cli/graphdblite-${VERSION}-${TARGET}.zip" \
    > "/dist/cli/graphdblite-${VERSION}-${TARGET}.zip.sha256"
...
(cd "$staging" && zip -q "/dist/ffi/graphdblite-ffi-${VERSION}-${TARGET}.zip" ./*)
sha256sum "/dist/ffi/graphdblite-ffi-${VERSION}-${TARGET}.zip" \
    > "/dist/ffi/graphdblite-ffi-${VERSION}-${TARGET}.zip.sha256"
rm -rf "$staging"
...
cp "target/${TARGET}/release/graphdblite_node.dll" \
    "/dist/node/graphdblite-${VERSION}.win32-x64-msvc.node"
